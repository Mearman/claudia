...
pub async fn migrate_agent_runs_to_session_ids(db: State<'_, AgentDb>) -> Result<String, String> {
    ...
    let mut stmt = conn
        .prepare("SELECT id, output FROM agent_runs WHERE session_id = '' AND output != ''")
        .map_err(|e| e.to_string())?;
        ...
        if !session_id.is_empty() {
            ...
            match conn.execute(
                ...
                Ok(_) => {
                    migrated_count += 1;
                    info!(
                        "Migrated agent_run {} with session_id {}",
                        run_id, session_id
                    );
                }
                Err(e) => {
...
#[tauri::command]
pub async fn kill_agent_session(db: State<'_, AgentDb>, run_id: i64) -> Result<bool, String> {
